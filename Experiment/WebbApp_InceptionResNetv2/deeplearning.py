...
folder_WebbApp = os.path.dirname(__file__)
...
def object_detection(path, filename):
    ...
    cv2.imwrite(
        '{}\static\predict/{}'.format(folder_WebbApp, filename), image_bgr)
    return coords

def save_text(filename, text):
    name, ext = os.path.splitext(filename)
    with open('{}\static\predict/{}.txt'.format(folder_WebbApp, name), mode='w') as f:
        f.write(text)
    f.close()
...
def OCR(path, filename):
    ...
    cv2.imwrite(
        '{}\static/roi/{}'.format(folder_WebbApp, filename), roi_bgr)
